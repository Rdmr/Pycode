...
class PySTC(stc.StyledTextCtrl):
    ...
    def onKeyPressed(self, event):
        ...
        #Ctrl+N
        if (key == 78) and event.ControlDown():
            self.parentFrame.OnNewTab([])
            
        #Ctrl+Z
        if (key == 90) and event.ControlDown():
            self.Undo()
    ...
    def Autoindent(self):
        indent = self.GetLineIndentation(self.GetCurrentLine()-1)

        if (len(self.GetLine(self.GetCurrentLine()-1)) > 2) and (self.GetLine(self.GetCurrentLine()-1)[-3] == ':') :
            indent += 4
		
        s = self.GetLine(self.GetCurrentLine()-1).strip()
        if (s.startswith('return') or s.startswith('pass')):
           indent -= 4
        if (indent < 0): intend = 0
           
        self.InsertText(self.GetCurrentPos(), ' '*indent)
        self.SetCurrentPos(self.GetCurrentPos() + indent)
        self.SetSelection(self.GetCurrentPos() + indent, self.GetCurrentPos() + indent)
